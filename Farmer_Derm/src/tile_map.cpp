...
namespace fd {
	...
	bool TileMap::Load(const sf::Texture &tileset, const char* map_filename) {
		...
		const int map_height = map_parser_.GetMapNumTilesInX();
		const int map_width = map_parser_.GetMapNumTilesInY();	
		...
		for (auto& cur_layer : map_parser_.GetLayers()) {
			...
			for (unsigned int i = 0; i < map_width; i++) {
				for (unsigned int j = 0; j < map_height; j++) {
					...
					int tu = cur_tile_number % (tileset_.getSize().x / map_parser_.GetTileWidthInPixels());
					int tv = cur_tile_number / (tileset_.getSize().y / map_parser_.GetTileWidthInPixels());

					// Get pointer to current tile's quad.
					sf::Vertex* quad = &vertices[(i + j * map_width) * 4];

					// Define the quad's 4 corners.
					quad[0].position = sf::Vector2f(i * map_parser_.GetTileWidthInPixels(), j * map_parser_.GetTileHeightInPixels());
					quad[1].position = sf::Vector2f((i + 1) * map_parser_.GetTileWidthInPixels(), j * map_parser_.GetTileHeightInPixels());
					quad[2].position = sf::Vector2f((i + 1) * map_parser_.GetTileWidthInPixels(), (j + 1) * map_parser_.GetTileHeightInPixels());
					quad[3].position = sf::Vector2f(i * map_parser_.GetTileWidthInPixels(), (j + 1) * map_parser_.GetTileHeightInPixels());	

					// Define the quad's 4 texture coordinates.
					quad[0].texCoords = sf::Vector2f(tu * map_parser_.GetTileWidthInPixels(), tv * map_parser_.GetTileHeightInPixels());
					quad[1].texCoords = sf::Vector2f((tu + 1) * map_parser_.GetTileWidthInPixels(), tv * map_parser_.GetTileHeightInPixels());
					quad[2].texCoords = sf::Vector2f((tu + 1) * map_parser_.GetTileWidthInPixels(), (tv + 1) * map_parser_.GetTileHeightInPixels());
					quad[3].texCoords = sf::Vector2f(tu * map_parser_.GetTileWidthInPixels(), (tv + 1) * map_parser_.GetTileHeightInPixels());	
				}
			}
